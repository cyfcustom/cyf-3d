...
export function AdminAuditPage() {
  ...
  const handleRevert = async (entry: AuditEntry) => {
    ...
    try {
      ...
      toast.success(`Cambio revertido en "${entry.module_name}.${entry.field_changed}"`);
      await loadAudit();
    } catch (error) {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground font-medium mt-2">
          Historial de todos los cambios realizados a las configuraciones.
        </p>
      </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-lg bg-red-500/5 border border-red-500/10">
                  <span className="font-semibold text-red-600 dark:text-red-400">Valor anterior</span>
                  <pre className="mt-1 text-muted-foreground overflow-x-auto max-h-24">
                    {JSON.stringify(entry.old_value, null, 2)}
                  </pre>
                </div>
                <div className="p-3 rounded-lg bg-green-500/5 border border-green-500/10">
                  <span className="font-semibold text-green-600 dark:text-green-400">Valor nuevo</span>
                  <pre className="mt-1 text-muted-foreground overflow-x-auto max-h-24">
