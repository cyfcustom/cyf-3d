...
type Props = {
  totals: ReturnType<typeof useSublimationCalculator>['totals'];
  onReset: () => void;
};

export function SublimationSummaryCard({ totals, onReset }: Props) {
  ...
  const copy = () => {
    const lines = [
      ...
      `${t('summaryLabels.subtotalNoGITotal')}: ${formatMoney(totals.subtotalBeforeFinancials)}`,
      `${t('summaryLabels.profitTotal')}: ${formatMoney(totals.totalProfit)}`,
      `${t('summaryLabels.taxTotal')}: ${formatMoney(totals.totalTax)}`,
  ...
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-lg font-semibold">{t('summaryModule.sublimation')}</h3>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={copy}>{t('actions.copy')}</Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        ...
        <Info label={t('summaryLabels.laborPerUnit')} value={formatMoney(totals.perUnit.labor)} />
