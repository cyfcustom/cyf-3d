...
type Props = {
  totals: ReturnType<typeof useEmpaquesCalculator>['totals'];
  onReset: () => void;
};

export function EmpaquesSummaryCard({ totals, onReset }: Props) {
  const copyBreakdown = () => {
    const lines = [
  ...
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-lg font-semibold">Resumen Empaques</h3>
