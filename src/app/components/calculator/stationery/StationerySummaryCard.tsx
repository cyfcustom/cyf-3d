...
type Props = {
  totals: ReturnType<typeof useStationeryCalculator>['totals'];
  onReset: () => void;
};

export function StationerySummaryCard({ totals, onReset }: Props) {
  ...
  const copyBreakdown = () => {
    const lines = [
      ...
      `${t('summaryLabels.indirectPerUnit')}: ${formatMoney(totals.perUnit.indirect)}`,
      `${t('summaryLabels.laborPerUnit')}: ${formatMoney(totals.perUnit.labor)}`,
      `${t('summaryLabels.subtotalNoGI')}: ${formatMoney(totals.perUnit.subtotalBeforeProfitAndTax)}`,
      `${t('summaryLabels.subtotalPerUnit')}: ${formatMoney(totals.perUnit.subtotal)}`,
      `${t('summaryLabels.profitTotalFull')}: ${formatMoney(totals.totalProfit)}`,
      `${t('summaryLabels.taxTotalFull')}: ${formatMoney(totals.totalTax)}`,
  ...
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-lg font-semibold">{t('summaryModule.stationery')}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        ...
        <Info label={t('summaryLabels.indirectPerUnit')} value={formatMoney(totals.perUnit.indirect)} />
        <Info label={t('summaryLabels.moPerUnit')} value={formatMoney(totals.perUnit.labor)} />
        <Info label={t('summaryLabels.subtotalNoGI')} value={formatMoney(totals.perUnit.subtotalBeforeProfitAndTax)} />
        <Info label={t('summaryLabels.subtotalPerUnit')} value={formatMoney(totals.perUnit.subtotal)} />
