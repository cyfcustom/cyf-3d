...
export function StationeryGICard({
  state,
  onProfitChange,
  onTaxChange,
  onToggleProfit,
  ...
  state: Pick<ReturnType<typeof useStationeryCalculator>['state'], 'profitPct' | 'taxPct' | 'applyProfit' | 'applyTax' | 'quantity'>;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2 border border-border rounded-xl p-4 bg-muted/20">
          ...
          <Input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => onProfitChange(parseMoney(e.target.value))}
          />
        </div>
        ...
        <div className="space-y-2 border border-border rounded-xl p-4 bg-muted/20">
          ...
          <Input
            ...
            inputMode="decimal"
            value={state.taxPct}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => onTaxChange(parseMoney(e.target.value))}
          />
        </div>
      </div>
