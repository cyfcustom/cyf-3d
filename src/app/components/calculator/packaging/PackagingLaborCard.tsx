...
export function PackagingLaborCard({
  state,
  onChange,
}: {
  state: ReturnType<typeof usePackagingCalculator>['state']['mo'];
    ...
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t('descriptions.laborPackaging')}</p>
        </div>
        <Switch checked={state.enabled} onCheckedChange={(v) => onChange({ enabled: v })} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label={t('fields.monthlySalary')} value={state.salary} onChange={(v) => onChange({ salary: v })} />
        <Field label={t('fields.hoursPerMonth')} value={state.hoursPerMonth} onChange={(v) => onChange({ hoursPerMonth: v })} />
        <Field label={t('fields.setupMin')} value={state.setupMinutes} onChange={(v) => onChange({ setupMinutes: v })} />
        <Field
      ...
      <div className="text-sm text-muted-foreground">{t('fields.laborPerUnit')}: {formatMoney(perUnit, 'USD')}</div>
    </div>
  );
}
