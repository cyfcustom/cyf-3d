...
function DepRow({
  ...
  item: ReturnType<typeof useLabelsCalculator>['state']['depreciation'][number];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-center">
      ...
      {item.mode === 'life' ? (
        <div className="space-y-1">
          <span className="text-xs text-muted-foreground">{t('fields.usefulLifeCuts')}</span>
          <Input
            type="number"
            inputMode="decimal"
            min="1"
...
export function LabelsDepreciationCard({
  items,
  onChange,
}: {
  items: ReturnType<typeof useLabelsCalculator>['state']['depreciation'];
  onChange: (
    id: string,
    patch: Partial<ReturnType<typeof useLabelsCalculator>['state']['depreciation'][number]>
  ) => void;
}) {
  const { t } = useTranslation('calculator');
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{t('descriptions.depreciationLabels')}</p>
      </div>
