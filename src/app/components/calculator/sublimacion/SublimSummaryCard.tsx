...
type Props = {
  totals: ReturnType<typeof useSublimacionCalculator>['totals'];
  onReset: () => void;
};

export function SublimSummaryCard({ totals, onReset }: Props) {
  const copy = () => {
    const lines = [
      ...
      `+ Ganancia: ${formatMoney(totals.gananciaTotal)}`,
      `+ Impuestos: ${formatMoney(totals.impuestosTotal)}`,
  ...
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-lg font-semibold">Resumen Sublimación</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        ...
        <Info label="Mano de obra (ud)" value={formatMoney(totals.perUnit.mo)} />
