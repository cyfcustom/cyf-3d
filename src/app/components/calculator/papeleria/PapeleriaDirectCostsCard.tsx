...
type Props = {
  items: ReturnType<typeof usePapeleriaCalculator>['state']['direct'];
  onChange: (id: string, patch: Partial<{ enabled: boolean; price: number; quantity: number }>) => void;
};

export function PapeleriaDirectCostsCard({ items, onChange }: Props) {
  ...
  return (
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Precio del paquete / unidades. Activa solo lo usado.</p>
      </div>
