...
type Props = {
  items: ReturnType<typeof useEsferasCalculator>['state']['depreciation'];
  onChange: (id: string, patch: Partial<{ enabled: boolean; price: number; unitsPerMonth?: number; lifeUnits?: number }>) => void;
};

export function EsferasDepreciationCard({ items, onChange }: Props) {
    ...
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Base 24 meses. Ajusta precio y, si aplica, unidades/mes o vida útil.</p>
        </div>
      </div>
          ...
          return (
            <div key={item.id} className="rounded-xl border border-border p-4 bg-muted/30 space-y-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-muted-foreground uppercase tracking-wide">{isLife ? 'Vida en usos' : 'Costo mensual'}</span>
              </div>
