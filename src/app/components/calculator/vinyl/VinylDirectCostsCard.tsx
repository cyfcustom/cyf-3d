...
type Props = {
  items: ReturnType<typeof useVinylCalculator>['state']['direct'];
  onChange: (id: string, patch: Partial<{ enabled: boolean; price: number; quantity: number }>) => void;
};

export function VinylDirectCostsCard({ items, onChange }: Props) {
    ...
    <div className="card bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t('descriptions.directCostsVinyl')}</p>
        </div>
      </div>
